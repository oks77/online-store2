export default class Pagination {
  constructor({ totalElements = 0, activePageIndex = 0, pageSize = 0 } = {}) {
    if (activePageIndex < 0 || activePageIndex > totalElements - 1) {
      throw Error(
        `Prop "activePageIndex" must be in range "0-${totalElements - 1}`
      );
    }
  ...
  setPage(pageIndex = 0) {
    ...
    if (
      pageIndex > this.maxRangePageIndex ||
      pageIndex < this.minRangePageIndex
    )
      return;
  ...
  addEventListeners() {
    const prevRangeBtn = this.element.querySelector(
      '[data-element="nav-prev-range"]'
    );
    const nextRangeBtn = this.element.querySelector(
      '[data-element="nav-next-range"]'
    );
